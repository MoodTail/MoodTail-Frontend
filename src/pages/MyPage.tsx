...
import { CHARACTER_GRADIENTS, CHARACTER_IMAGES, type CharacterType } from '../constants/characters'
import chevronRightIcon from '../assets/icons/chevron-right.svg'
import '../styles/MyPage.css'

function MenuArrow() {
  return <img className="mypage__menu-arrow" src={chevronRightIcon} alt="" aria-hidden="true" />
}
...
type ModalStep = 'none' | 'logout-confirm' | 'logout-done' | 'withdraw-confirm' | 'withdraw-done' | 'login-prompt'
...
function MyPage({ onEditProfile, onInquiry, onTerms }: MyPageProps) {
  ...
  const { isLoggedIn, nickname, characterType, testCount, monthlyCount, collectedCount } = MOCK_USER
  ...
  return (
    <div className="mypage">
      <section
        ...
        style={isLoggedIn ? { background: CHARACTER_GRADIENTS[characterType] } : undefined}
      >
        {isLoggedIn ? (
          <>
            <div className="mypage__avatar" aria-hidden="true">
              {CHARACTER_IMAGES[characterType] ? (
                <img className="mypage__avatar-image" src={CHARACTER_IMAGES[characterType]} alt="" />
              ) : (
                '🍹'
              )}
            </div>
          ...
          >
            <div className="mypage__avatar mypage__avatar--empty" aria-hidden="true">
              <span className="mypage__nickname mypage__nickname--guest">로그인이 필요해요!</span>
            </div>
      ...
      <div className="mypage__panel">
        ...
        <nav className="mypage__menu" aria-label="마이페이지 메뉴">
          ...
          <button type="button" className="mypage__menu-item" onClick={handleInquiry}>
            <span>문의하기</span>
            <MenuArrow />
          </button>

          <button type="button" className="mypage__menu-item mypage__menu-item--muted" onClick={handleTerms}>
            <span>서비스 이용약관</span>
            <MenuArrow />
          </button>
      ...
      {modalStep === 'logout-confirm' && (
        <Modal
          title="정말 로그아웃하시겠어요?"
          buttons={[
            { label: '로그아웃', onClick: handleLogout, variant: 'secondary' },
            { label: '닫기', onClick: closeModal, variant: 'primary' },
          ]}
        />
      )}

      {modalStep === 'logout-done' && (
        <Modal title="로그아웃 되었습니다" buttons={[{ label: '닫기', onClick: closeModal, variant: 'primary' }]} />
      )}

      {modalStep === 'withdraw-confirm' && (
        <Modal
          title="정말 탈퇴하시겠어요?"
          description={'탈퇴 시 모든 데이터가 삭제되며,\n복구할 수 없습니다.'}
          buttons={[
            { label: '탈퇴하기', onClick: handleWithdraw, variant: 'secondary' },
            { label: '닫기', onClick: closeModal, variant: 'primary' },
          ]}
        />
      )}
      ...
      {modalStep === 'withdraw-done' && (
        <Modal
          title="탈퇴가 완료되었습니다"
          buttons={[{ label: '닫기', onClick: closeModal, variant: 'primary' }]}
        />
      )}

      {modalStep === 'login-prompt' && (
        <Modal
          className="modal--login-prompt"
          title="로그인하고 기록을 저장해요"
          description={'테스트 결과, 도감, 즐겨찾기를\n이어서 사용할 수 있어요.'}
          buttons={[
            { label: '로그인하기', onClick: handleGoToLogin, variant: 'primary' },
            { label: '닫기', onClick: closeModal, variant: 'secondary' },
          ]}
        />
      )}
    </div>
