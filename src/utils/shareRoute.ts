...
export function parseSharedRoute(): SharedRoute | null {
  ...
  if (collectionMatch)
    return { type: "collection", shareToken: collectionMatch[1] };

  const pairMatch = pathname.match(/^\/share\/pair\/([^/]+)$/);
  if (pairMatch) return { type: "pair", shareToken: pairMatch[1] };
